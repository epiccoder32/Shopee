...
const AppContext=({children})=>{
    ...
    const handleProductQuanity=({item,type})=>{
        ...
        if(type=='inc')
        {
            pitems[index].quantity +=1
        }
        else
        {
            if( pitems[index]?.quantity>1)
            {
                pitems[index].quantity -=1
            }
    ...
    const getCategories=()=>{
    const categories={
      data:[{id:1,img:Category1Img, name:'HEADPHONES'},{id:2,img:Category2Img,name:'BLUETOOTH SPEAKERS'},{id:3,img:Category3Img, name:'SMART WATCHES'},{id:4,img:Category4Img,name:'EARPODS'}]
    };
    return categories
}
const getProducts=()=>{
  ...
  {
    id:8,img:headphone5,categoryid:1, 
  ...
  {
    id:9,img:headphone6,categoryid:1, 
